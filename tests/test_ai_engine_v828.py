...
checks={
 'version 8.2.8':version.get('version')=='8.2.8',
 'scripts versioned':'js/v8-ai-assistant.js?v=8.2.8' in index and 'js/v8-seller-settlement.js?v=8.2.8' in index,
 'hybrid engine':'answerWithEngine' in ai and 'businessSnapshot' in ai and 'local-fallback' in ai,
 'operational drafts':all(x in ai for x in ['create_payment_plan','generate_receipt','seller_settlement','resolveDraftActionV828','parseRequestedItemsV828']),
 ...
 'cache version':"nv-app-shell-v828" in sw and 'v8-seller-settlement.js' in sw,
 ...
 'multi item operational sale':all(x in ai for x in ['parseRequestedItemsV828','isNonBlockingSaleFieldV828','operationalReady','primaryWork','productOptionsV828']),
 ...
 'functional sanitation':all(x in quality for x in ['buildFunctionalHealthV828','Saneamiento funcional','nv828RecheckFunctions']) and '.nv828FunctionSummary' in css,
 'workflow current engine test':'tests/test_ai_engine_v828.py' in workflow and 'test_ai_engine_v823.py' not in workflow,
 'under 100 files':len(files)<=100,
}
failed=[k for k,v in checks.items() if not v]
if failed:
 print('FALLOS:',failed,'ARCHIVOS:',len(files)); sys.exit(1)
print(f'Motor IA y caja V8.2.8: {len(checks)}/{len(checks)} controles OK · {len(files)} archivos')
