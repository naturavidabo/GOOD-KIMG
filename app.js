...
const modules = [
  ...
  ['dashboard','◉','Control remoto','Estado del negocio, caja, ventas, faltantes y sincronización desde cualquier lugar.'],
  ['reports','▥','Reportes','Resumen diario, mensual y documentos imprimibles.'],
  ['users','♙','Usuarios','Administrador, propietaria y ayudante.'],
  ['settings','⚙','Configuración','Respaldo, verificación, almacenamiento, sincronización y mantenimiento.']
];
...
function openDB() {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = event => {
      ...
      const conflictStore = transaction.objectStore('syncConflicts');
      if (!conflictStore.indexNames.contains('status')) conflictStore.createIndex('status', 'status', { unique: false });
      if (!conflictStore.indexNames.contains('entity')) conflictStore.createIndex('entity', 'entity', { unique: false });
...
function renderSideLinks() {
  const restricted = new Set(['clients','products','recipes','inventory','market','purchases','expenses','dashboard','reports','users','settings']);
